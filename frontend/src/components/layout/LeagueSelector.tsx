...
export default function LeagueSelector() {
  ...
  const currentLeagueId = searchParams.get('league') ?? leaguesConfig.active_league
  const currentLeague = leaguesConfig.leagues.find((l) => l.id === currentLeagueId)
      ...
      <div className="flex items-center justify-center gap-3 mt-4 flex-wrap">
        ...
        >
          {leaguesConfig.leagues.map((lg) => {
            const label = lg.display_name ? `${lg.name} — ${lg.display_name}` : lg.name
            const statusSuffix = lg.status !== 'active' ? ` (${lg.status})` : ''
            return (
